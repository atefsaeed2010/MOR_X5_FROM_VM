...
#-------- Includes -------------------------
if [ -f /usr/src/mor/test/node2/nnode_functions.sh ]; then
    . /usr/src/mor/test/node2/nnode_functions.sh
fi
...
if [ "$firstParam" == "-l" ]; then
    ...
    #====== MOR/CRM GUI prepare
        ...
        echo "Preparing GUI and DB: $TEST_MOR_VERSION Revision: $TEST_REVISION";
        cleanup_various_files &
        prepare_gui

        echo "Preparing DB for $TEST_MOR_VERSION Revision: $TEST_REVISION"
       
        prepare_db
        echo "Done. Prepared: $TEST_MOR_VERSION Revision: $TEST_REVISION";
