...
. $SCRIPTS_DIR/framework/settings.sh
...
mysql_check_add_grant()
{
	...
	if [ "$MYSQL_GRANT_IS_PRESENT" == "1" ]; then
		...
		if [ "$MYSQL_GRANT_IS_PRESENT" == "1" ]; then
			...
			report "Failed to add $GRANT grant for MOR DB user. Try to run this script directly in DB server: /usr/src/mor/x5/mysql/mysql_grants.sh to fix this problem. If problem remains - contact Kolmisoft"  1
			GRANT_IS_PRESENT="1"
      	fi
