...
#define BUFFER_SIZE         SQL_BATCH_SIZE * 300 + 500
...
void calculate_call_price(call_data *node);
int get_cached_rate(call_data *data, int tariff_id, char *prefix, int reseller);
...
int main(int argc, char *argv[]) {
    ...
    mor_init("Starting MOR CDR Rerate script\n");
...
void calls_rerate() {
    ...
    while (current != NULL) {
        ...
        int user_cached_index = -1;
        int reseller_cached_index = -1;

        if (!(current->reseller_id && !include_reseller_users) || user_belongs_to_reseller) {
            if (strlen(current->prefix)) {
                pthread_mutex_lock(&mutex);
                user_cached_index = get_cached_rate(current, current->user_tariff, current->prefix, 0);
        ...
        if (current->reseller_id && user_belongs_to_reseller == 0) {
            if (strlen(current->prefix)) {
                pthread_mutex_lock(&mutex);
                reseller_cached_index = get_cached_rate(current, current->reseller_tariff, current->prefix, 1);
                pthread_mutex_unlock(&mutex);
            }
        } else {
        ...
        if (user_cached_index == -1 || reseller_cached_index == -1) {
    ...
    while (current != NULL) {
        ...
        if (DEBUG_RERATE == 0 && (user_need_update || reseller_need_update)) {
            ...
            sprintf(buffer, "(%lld,%.6f,%.6f,%d,%.6f,%.6f,%d),", current->call_id, uprice, urate, ubillsec, rprice, rrate, rbillsec);
        ...
        if (DEBUG_RERATE == 0 && (batch_counter == SQL_BATCH_SIZE)) {
            ...
            memset(update_query, 0, BUFFER_SIZE);
...
int calls_get(int i) {
    ...
    sprintf(sqlcmd, "SELECT calls.id, user_price, user.tariff_id, calldate, billsec, user_billsec, user_rate, "
                    "prefix, calls.user_id, tariffs.purpose, dst, devices.location_id, devices.grace_time, "
                    "currencies.exchange_rate, reseller_id, reseller_price, reseller.tariff_id, reseller_billsec, reseller_rate, "
                    "reseller_tariffs.purpose, reseller_currencies.exchange_rate FROM calls "
                    ...
                    "WHERE %s calldate BETWEEN '%s' AND '%s' "
                    "AND calls.id %s %lld AND disposition='ANSWERED' AND provider_id > 0 "
                    "AND real_billsec > 0 LIMIT %d", user_sql, date_from, date_till, sign, last_call_id, RERATE_BACTHES);
        ...
        if (result) { // there are rows
            ...
            while ((row = mysql_fetch_row(result))) {
                ...
                if (row[0]) {
                    ...
                    node->real_billsec = (int)ceilf(atoi(row[4]) + (atoi(row[4]) * (ghost_time/100)));
                    ...
                    if (row[12]) node->user_grace_time = atoi(row[12]); else node->user_grace_time = 0;
                    if (row[13]) node->exchange_rate = atof(row[13]); else node->exchange_rate = 1;
                    ...
                    if (row[20]) node->exchange_rate = atof(row[20]); else node->exchange_rate = 1;
                    ...
                    if (!(node->reseller_id && !include_reseller_users) || user_belongs_to_reseller) {
                        // calculate user price for single user
...
void *get_rate_details(void *arg) {
    ...
    int check_user = 0;
    int check_reseller = 0;
    int reseller_checked = 0;
    ...
    if (args->user_cached_rate_index == -1) check_user = 1;
    if (args->reseller_cached_rate_index == -1) check_reseller = 1;

    // wholesale

    check_reseller_label:

    if (check_reseller) tariff_type = args->data->reseller_tariff_type;
    if (check_user) tariff_type = args->data->user_tariff_type;
    ...
    if (check_user) {
        check_user = 0;
        cached_rates_function(args->data, 0);
    }

    if (check_reseller == 1 && reseller_checked == 1) {
        check_reseller = 0;
        cached_rates_function(args->data, 1);
    }

    if (check_user == 0 && check_reseller == 0) {
        get_rate_details_sem--;
    } else if (check_reseller == 1 && reseller_checked == 0) {
        pthread_mutex_unlock(&mutex);
        goto check_reseller_label;
    }
...
void calculate_call_price(call_data *node) {
    ...
    if (check_user) {
        /* check grace time */
        if (node->user_grace_time > 0) {
            if (node->user_grace_time >= node->real_billsec) {
                old_real_billsec = node->real_billsec;
                node->real_billsec = 0;
            }
        }
    }

    check_reseller_price_label:

    i = 0;
    li = 0;
    t_billsec = 0;

    if (check_user == 0 && check_reseller == 1 && old_real_billsec > 0) {
        node->real_billsec = old_real_billsec;
    }
    ...
    if (tariff_type == 1) {
        ...
        for (i = 0; i < total_arates; i++) {
            ...
            if (arates[i].from <= node->real_billsec) {
                ...
                if (arates[i].artype == 1) {
                    ...
                    if (arates[i].duration == -1 ) {
                        ...
                        t_billsec = node->real_billsec - arates[i].from + 1;
                    } else {
                        if (node->real_billsec < (arates[i].from + arates[i].duration) ) {
                            t_billsec = node->real_billsec - arates[i].from + 1;
                        } else {
                    ...
                    *call_price += (arates[i].price * t_billsec) / 60 ;

                } else {
                    /* event */
                    *call_price += arates[i].price;
                    t_billsec = 0;
                }
        ...
        if (total_arates > 0)
            if ((t_billsec + arates[li].from - 1) > node->real_billsec)
                *call_billsec = t_billsec + arates[li].from - 1;
            else
                *call_billsec = node->real_billsec;
        else
            *call_billsec = node->real_billsec;
        ...
        *call_rate = max_arate;
        ...
        // possible error fixing
        if (increment_ws < 1) increment_ws = 1;

        // count seconds for user wholesale
        if (!(node->real_billsec % increment_ws)) {
            *call_billsec = ceilf(node->real_billsec / increment_ws) * increment_ws;
        } else {
            *call_billsec = (ceilf(node->real_billsec / increment_ws) + 1 ) * increment_ws;
        }

        if (( min_time_ws) && ( *call_billsec < min_time_ws )) *call_billsec = min_time_ws;

        *call_price = (rate_ws * *call_billsec) / 60;
        *call_price += connection_fee_ws;
        *call_price = *call_price / node->exchange_rate;
        *call_rate = rate_ws / node->exchange_rate;
        max_arate = rate_ws / node->exchange_rate;

    }

    if (check_user) check_user = 0;
    if (reseller_checked == 0 && check_reseller == 1) {
        goto check_reseller_price_label;
    }
...
int get_cached_rate(call_data *data, int tariff_id, char *prefix, int reseller) {
    ...
    if (reseller) {
        if (data->reseller_tariff_type == 0) {
    ...
    for (i = 0; i < cached_rates_count; i++) {
        if ((cached_rates[i].tariff_id == tariff_id) && (strcmp(prefix, cached_rates[i].prefix) == 0)) {
            ...
            if (reseller) {
                if (data->reseller_tariff_type == 1) {
                    int j = 0;
...
void get_calls_count() {
    ...
    sprintf(sqlcmd, "SELECT count(calls.id), min(calls.id) FROM calls "
                    ...
                    "WHERE %s calldate BETWEEN '%s' AND '%s' AND disposition='ANSWERED' AND provider_id > 0 AND real_billsec > 0", user_sql, date_from, date_till);
