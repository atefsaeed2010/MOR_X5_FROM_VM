...
int get_alerts() {
    ...
    if (mor_mysql_query("SELECT alerts.id, alert_type, alert_count_type, check_type, check_data, value_at_alert, "
                        ...
                        "enable_provider_in_lcr, clear_after, clear_on_date, notify_to_user, hgc, "
                        "GROUP_CONCAT(alert_dependencies.alert_id), alert_when_more_than, clear_when_less_than, name "
                        "FROM alerts "
                        "LEFT JOIN alert_dependencies ON alert_dependencies.owner_alert_id = alerts.id "
                        "WHERE status = 1 "
                        "GROUP BY alerts.id")) {
    ...
    while ((row = mysql_fetch_row(result)) != NULL) {
        ...
        if (row[34]) sprintf(alerts[alerts_count].alert_group_id_list, ",%s,", row[34]);
        if (row[35]) alerts[alerts_count].alert_if_more_than = atoi(row[35]);
        if (row[36]) alerts[alerts_count].clear_if_less_than = atoi(row[36]);
        if (row[37]) strcpy(alerts[alerts_count].name, row[37]);
    ...
    // get user alerts
    mor_mysql_query("UPDATE conflines SET value = '0' WHERE name = 'alerts_need_update'");
...
int get_calls_data() {
    ...
    sprintf(sqlcmd, "SELECT calls.id, user_id, provider_id, disposition, duration, billsec, "
                    ...
                    "hangupcause, users.ignore_global_alerts FROM calls "
    ...
    // check if there are alerts with active calls
    unsigned long long int i = 0;
    int prov_sim = 0;
    int user_sim = 0;
    for (i = 0; i < alerts_count; i++) {
        if (alerts[i].alert_type == 10) {
            if (alerts[i].check_type == 1) user_sim = 1;
            if (alerts[i].check_type == 2) prov_sim = 1;
        }
    }
    ...
    if (user_sim) {
        ...
        sprintf(sqlcmd, "SELECT user_id, count(activecalls.id) as active_calls, postpaid FROM activecalls "
    ...
    if (prov_sim) {
        // get active calls for provider
        sprintf(sqlcmd, "SELECT count(activecalls.id) as active_calls, provider_id FROM activecalls "
                        "GROUP BY provider_id "
                        "ORDER BY provider_id, active_calls DESC");
        ...
        while ((row = mysql_fetch_row(result)) != NULL) {
            ...
            if (row[0]) {
                if (last_prov_id != atoi(row[0])) {
                    if (atoi(row[1]) > 1) {

                        calls_data = realloc(calls_data, (active_calls_data_count + 1) * sizeof(calls_data_t));
                        memset(&calls_data[active_calls_data_count], 0, sizeof(calls_data_t));

                        if (row[0]) calls_data[active_calls_data_count].prov_sim_calls = atoi(row[0]);
                        if (row[1]) calls_data[active_calls_data_count].provider_id = atoi(row[1]);
                        calls_data[active_calls_data_count].all = 1;

                        last_prov_id = calls_data[active_calls_data_count].provider_id;
                        active_calls_data_count++;
    ...
    for (i = calls_data_count; i < active_calls_data_count; i++) {
        char line_buffer[512] = "";
        sprintf(line_buffer, "%i,%i,%i,%i,%i,%i\n", calls_data[i].user_id, calls_data[i].user_sim_calls, calls_data[i].postpaid, calls_data[i].user_lcr_id, calls_data[i].provider_id, calls_data[i].prov_sim_calls);
        fprintf(fp, line_buffer);
    }
    fclose(fp);
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        if (strcmp(line, "-- ACTIVE CALLS --\n") != 0 && continue_reading == 1) {
            ...
            while (pch != NULL) {
                ...
                if (tok_count == 6) calls_data[active_calls_data_count].prov_sim_calls = atoi(pch);
                ...
                if (data_node != NULL) {
                    ...
                    if (update) {
                        if (alert->check_type == 1) {
                            ...
                            data_node->data_sum = (float)calls_data[i].prov_sim_calls;
                            data_node->data_count = calls_data[i].prov_sim_calls;
                        }
                    }
...
int action_alert_email(alerts_t *alert, data_info_t *data, int report, char *cause, double data_sum, long int data_count) {
    ...
    sprintf(body, "Alert: %s\n\nReporting: %s\nParameter: %s\nGroup to notify: %s\nObject Type: %s\nObject: %s\nObject name: %s\nAlert ID: %u\n\n"
            ...
            alert->name, alertstr, cause, alert->group->name == NULL ? "" : alert->group->name,
            object_type, alert->check_data, object_name, alert->id, date_str, data_sum, data_count,
...
int action_alert_email_to_user(alerts_t *alert, data_info_t *data, int report, char *cause, double data_sum, long int data_count) {
    ...
    sprintf(body, "Alert: %s\n\nReporting: %s\nParameter: %s\nObject Type: %s\nObject: %s\nObject name: %s\nAlert ID: %u\n\n"
            ...
            object_name, alert->id, date_str, data_sum, data_count,
            alert->alert_if_more, alert->alert_if_less,
            alert->clear_if_more, alert->clear_if_less,
            alert->ignore_if_calls_more, alert->ignore_if_calls_less, alert->comment);
