...
void *mor_aggregate(void *args) {
    ...
    for (i = 0; i < calls_data_count; i++) {
        ...
        if (type == 1) {
            ...
            if (user_id != calls[i].user_id && count) got_data = 1;
        }

        // AGGREGATE BY TERMINATOR
        if (type == 2) {
            if (!(calls[i].terminator_id > 0)) goto skip;
            if (terminator_id != calls[i].terminator_id && count) got_data = 1;
        }

        // AGGREGATE BY DIRECTION
        if (type == 3) {
            if (!(calls[i].direction_id > 0)) goto skip;
            if (direction_id != calls[i].direction_id && count) got_data = 1;
        }

        // AGGREGATE BY ORIGINATOR AND TERMINATOR
        if (type == 4) {
            if (!(calls[i].user_id > 0) || !(calls[i].terminator_id > 0)) goto skip;
            if ((user_id != calls[i].user_id || terminator_id != calls[i].terminator_id) && count) got_data = 1;
        }

        // AGGREGATE BY ORIGINATOR AND DIRECTION
        if (type == 5) {
            if (!(calls[i].user_id > 0) || !(calls[i].direction_id > 0)) goto skip;
            if ((user_id != calls[i].user_id || direction_id != calls[i].direction_id) && count) got_data = 1;
        }

        // AGGREGATE BY TERMINATOR AND DIRECTION
        if (type == 6) {
            if (!(calls[i].terminator_id > 0) || !(calls[i].direction_id > 0)) goto skip;
            if ((terminator_id != calls[i].terminator_id || direction_id != calls[i].direction_id) && count) got_data = 1;
        }

        // AGGREGATE BY DIRECTION AND DESTINATION
        if (type == 7) {
            if (!(calls[i].direction_id > 0) || !(calls[i].destination_id > 0)) goto skip;
            if ((direction_id != calls[i].direction_id || destination_id != calls[i].destination_id) && count) got_data = 1;
        }
        ...
        if (type == 8) {
            ...
            if ((user_id != calls[i].user_id || terminator_id != calls[i].terminator_id || direction_id != calls[i].direction_id) && count) got_data = 1;
        }
        ...
        if (type == 9) {
            ...
            if ((user_id != calls[i].user_id || direction_id != calls[i].direction_id || destination_id != calls[i].destination_id) && count) got_data = 1;
        }
        ...
        if (type == 10) {
            ...
            if ((terminator_id != calls[i].terminator_id || direction_id != calls[i].direction_id || destination_id != calls[i].destination_id) && count) got_data = 1;
        }
        ...
        if (type == 11) {
            ...
            if ((user_id != calls[i].user_id || terminator_id != calls[i].terminator_id || direction_id != calls[i].direction_id || destination_id != calls[i].destination_id) && count) got_data = 1;
        }

        // UPDATE AGGREGATED DATA
        if (got_data) {
            mor_aggregate_sql_format(user_id, user_price, user_billsec, terminator_id, terminator_price, terminator_billsec, billsec, real_billsec, answered, prefix, direction, destination, count, user_count, type, tphid, tpdid, tpmid, user_price_tax);
        ...
        if (type == 3 || type == 5 || type == 6 || type == 7 || type == 8 || type == 9 || type == 10 || type == 11) {
            ...
            strcpy(direction, calls[i].direction_name);
        }
...
void mor_aggregate_sql_format(int o_id, double o_price, long int o_billsec, int t_id, double t_price, long int t_billsec, long int billsec, double real_billsec, int answered, char *prefix, char *direction, char *destination, int calls_count, int user_calls_count, int type, uint64_t tphid, uint64_t tpdid, uint64_t tpmid, double o_price_tax) {
    ...
    if (strlen(direction) > 0) strcpy(direction_str, direction);

    // lock thread because other aggregate threads might be running
    pthread_mutex_lock(&mutex);

    // for hours
    sprintf(buffer, "%s,%s,%s,%d,%s,%" PRIu64, o_id_str, t_id_str, prefix_str, type, direction_str, tphid > 0 ? tphid : time_period_hour_id);
    crc = crc64((const void *)buffer, strlen(buffer));
    sprintf(tmp_buffer, "(%" PRIu64 ",%d,%0.5f,%ld,%d,%0.5f,%ld,%ld,%.2f,%d,%d,'%s',%d,'%s','%s',%d,%" PRIu64 ",%0.5f),", crc, o_id, o_price, o_billsec, t_id, t_price, t_billsec, billsec, real_billsec, answered, calls_count, prefix, type, direction, destination, user_calls_count, tphid > 0 ? tphid : time_period_hour_id, o_price_tax);
    strcat(insert_update_values_query, tmp_buffer);
    // for days
    sprintf(buffer, "%s,%s,%s,%d,%s,%" PRIu64, o_id_str, t_id_str, prefix_str, type, direction_str, tpdid > 0 ? tpdid : time_period_day_id);
    crc = crc64((const void *)buffer, strlen(buffer));
    sprintf(tmp_buffer, "(%" PRIu64 ",%d,%0.5f,%ld,%d,%0.5f,%ld,%ld,%.2f,%d,%d,'%s',%d,'%s','%s',%d,%" PRIu64 ",%0.5f),", crc, o_id, o_price, o_billsec, t_id, t_price, t_billsec, billsec, real_billsec, answered, calls_count, prefix, type, direction, destination, user_calls_count, tpdid > 0 ? tpdid : time_period_day_id, o_price_tax);
    strcat(insert_update_values_query, tmp_buffer);
    // for months
    sprintf(buffer, "%s,%s,%s,%d,%s,%" PRIu64, o_id_str, t_id_str, prefix_str, type, direction_str, tpmid > 0 ? tpmid : time_period_month_id);
    crc = crc64((const void *)buffer, strlen(buffer));
    sprintf(tmp_buffer, "(%" PRIu64 ",%d,%0.5f,%ld,%d,%0.5f,%ld,%ld,%.2f,%d,%d,'%s',%d,'%s','%s',%d,%" PRIu64 ",%0.5f),", crc, o_id, o_price, o_billsec, t_id, t_price, t_billsec, billsec, real_billsec, answered, calls_count, prefix, type, direction, destination, user_calls_count, tpmid > 0 ? tpmid : time_period_month_id, o_price_tax);
    strcat(insert_update_values_query, tmp_buffer);
    batch_count += 3;
    current_aggregates += 3;
    // printf("o_id: %d, o_price: %.2f, o_billsec: %ld, t_id: %d, t_price: %.2f, t_billsec: %ld, billsec: %ld, real_billsec: %.2f, answered: %d, prefix: %s, direction: %s, destination: %s, calls_count: %d, user_calls_count: %d, crc: %" PRIu64 "\n", o_id, o_price, o_billsec, t_id, t_price, t_billsec, billsec, real_billsec, answered, prefix, direction, destination, calls_count, user_calls_count, crc);
...
int mor_aggregate_get_current_date(char *date, int mode) {
    ...
    if (mode > 0) {
        if (mode == 1) return tmp.tm_hour;
        if (mode == 2) return tmp.tm_mday;
        if (mode == 3) return tmp.tm_mon + 1;
    }
...
int mor_compare_date(char *date) {
    ...
    mor_aggregate_get_current_date(current_date, 0);
    ...
    strptime(current_date, DATE_FORMAT, &tm0);
    strptime(date, DATE_FORMAT, &tm1);
    strptime(last_calldate, DATE_FORMAT, &tm2);
    ...
    if (!strlen(last_calldate)) {
        if (t1 >= t0) strcpy(last_calldate, date);
        return 0;
    }

    if (t1 > t2 && t1 >= t0) {
        strcpy(last_calldate, date);
    }
...
uint64_t mor_get_time_period_id(int type, int missing_period_index, char *calldate) {
    ...
    mor_aggregate_get_current_date(time_buffer1, 0);
    mor_aggregate_get_current_date(time_buffer2, 0);

    if (missing_period_index > -1) {
        strcpy(time_buffer1, missing_time_periods[missing_period_index].date);
        strcpy(time_buffer2, missing_time_periods[missing_period_index].date);
    }
    ...
    sprintf(query, "SELECT id, from_date FROM time_periods WHERE %s", type_str);
    ...
    if (result) {
        if (mysql_num_rows(result)) {
            ...
            if (type == 1 && missing_period_index == -1) if (row[1]) strcpy(aggregate_stopped_at, row[1]);
        }
    }
...
/*
    Finished time period looks like this 'xxxx-xx-xx xx:59:59' in other words, just set minutes and second to '59' (only for hour periods)
*/
