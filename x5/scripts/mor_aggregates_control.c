...
int main(int argc, char *argv[]) {
    ...
    // starting sript
    mor_init("Starting MOR Aggregates Control script\n");
    ...
    if (missing_time_periods_count) {
        ...
        for (i = 0; i < missing_time_periods_count; i++) {
            mor_log("Date: %s, Full aggregate: %d\n", missing_time_periods[i].date, missing_time_periods[i].full_aggregate);
        }
        ...
        for (i = 0; i < missing_time_periods_count; i++) {
            ...
            if ((time_period_hour_id = mor_get_time_period_id(1, i, NULL)) == 0) {
                mor_log("Hour time period id = 0. Aborting\n");
                exit(1);
            }

            // get period id for this day
            if ((time_period_day_id = mor_get_time_period_id(2, i, NULL)) == 0) {
                mor_log("Day time period id = 0. Aborting\n");
                exit(1);
            }

            // get period id for this month
            if ((time_period_month_id = mor_get_time_period_id(3, i, NULL)) == 0) {
        ...
        for (j = 0; j < hour_diff; j++) {
            ...
            // get period id for this hour
            if ((time_period_hour_id = mor_get_time_period_id(1, -1, calculated_date)) == 0) {
                mor_log("Hour time period id = 0. Aborting\n");
                exit(1);
            }

            // get period id for this day
            if ((time_period_day_id = mor_get_time_period_id(2, -1, calculated_date)) == 0) {
                mor_log("Day time period id = 0. Aborting\n");
                exit(1);
            }

            // get period id for this month
            if ((time_period_month_id = mor_get_time_period_id(3, -1, calculated_date)) == 0) {
                mor_log("Month time period id = 0. Aborting\n");
                exit(1);
            }
...
int mor_get_calls_data(int time_period_index, char *calldate) {
    ...
    if (calldate != NULL) {
        ...
        // if period is marked for full aggregate, then we need to make from date xxxx-xx-xx xx:00:00 and till date xxxx-xx-xx xx:59:59
        // if period is nor marked for full aggregate, then we need to make only till date xxxx-xx-xx xx:59:59, because date passed in the missing_time_periods variable
        // will have the date from already in the correct time
        if (missing_time_periods[time_period_index].full_aggregate) {
            strcpy(date_from + 14, "00:00");
            strcpy(date_till + 14, "59:59");
        } else {
            strcpy(date_till + 14, "59:59");
...
int mor_get_missing_periods() {
    ...
    sprintf(query, "SELECT from_date FROM time_periods WHERE period_type = 'hour' AND from_date < '%s'", current_time);
        ...
        if (mysql_num_rows(result)) {
            while (( row = mysql_fetch_row(result) )) {
                if (row[0]) {
                    ...
                    strcpy(time_periods[time_periods_count].date, row[0]);
                    time_periods_count++;
                }
            }
        }
...
int get_oldest_time_period() {
    ...
    if (mor_mysql_query("SELECT MIN(from_date) FROM time_periods WHERE period_type = 'hour'")) {
        return 1;
    }
