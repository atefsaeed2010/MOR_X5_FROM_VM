...
# Usage:
#
#   1. Copy this script to the directory where all the source files are located
...
if [ $? -eq 0 ]; then
    ...
    RESP2=
    report "MOR GUI not found. Please enter Core version: (12/x3/x4/x5)" 3
    read -t 60 RESP2

    if [ "$RESP2" != "x3" ] && [ "$RESP2" != "x4" ] && [ "$RESP2" != "x5" ] && [ "$RESP2" != "12" ]; then
...
if [ $MOR_VERSION == "x3" ] || [ $MOR_VERSION == "12.126" ] || [ $MOR_VERSION == "x4" ] || [ $MOR_VERSION == "x5" ]; then
